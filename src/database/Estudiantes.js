...
const getOneStudent = async (id) => {
  try {
    const res = await pool.query('select a.id,a.nombre,a.matricula,a.sexo,a.licenciatura,ca.url_imagen from alumnos a inner join cuenta_alumnos ca on a.matricula=ca.matricula where a.matricula=$1', [id]);
    return res.rows[0];
  } catch (error) {
    throw error;
  }
...
const createCuenta=async(cuenta)=>{
  try {
    ...
    if (result.rows.length === 0) {
      ...
      await pool.query('INSERT INTO cuenta_alumnos VALUES ($1, $2, $3, $4);', [matricula, correo, contraseña, url_imagen]);
